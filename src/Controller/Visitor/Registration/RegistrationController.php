...
class RegistrationController extends AbstractController
{
    ...
    {
        // 1- Créons l'utilisator à insérer en base de données
        $user = new User();

        // 2- Créons le formulaire d'inscription
        $form = $this->createForm(RegistrationFormType::class, $user);

        // 4- Associons au formulaire les données de la requete
        $form->handleRequest($request);

        // 5- Si le formulaire est soumus et valide
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // 8- Demndons au manager des entités de préparer la requete d'insertion de l'utilisateur qui s'insscrit en base de données
            $entityManager->persist($user);

            // 9- Executons la requete
            $entityManager->flush();
            ...
            // 11- Rediger l'utilisateur vers la page d'accueil
            return $this->redirectToRoute('visitor_registration_waiting_for_email_verification');
        }
        //3- Passons le formulaire à la page pour afficher
    ...
    #[Route('register/waiting_for_email_verification', name: 'visitor_registration_waiting_for_email_verification', methods: ['GET'])]
    ...
    public function verifyUserEmail(Request $request, TranslatorInterface $translator, UserRepository $userRepository): Response
    {
        ...
        $this->addFlash('success', 'Votre compte a été verifié, vous pouvez vous connecter.');
